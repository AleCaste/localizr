...
var prototype = {
    ...
    load: function (callback) {
        ...
        try {
            ...
            var step2 = function() {
              if (i18n_cache!=null && (data_cached = i18n_cache.get(''+that.file))!=null) {
                data_file = data_cached;
                step3();
                return;
              } else {
                if (gv.file_exists(that.file)==false)  { if (i18n_cache!=null)  i18n_cache.del(''+that.file); data_file = {}; step3(); return; }
                spud.deserialize(fs.createReadStream(that.file), that.type, function (err, data) {
                  if (err) {
                    callback(err);
                    return;
                  }
                  if (i18n_cache!=null)  i18n_cache.set(''+that.file, data);
                  data_file = data;
                  step3();
                });
              }
            ...
            var step1 = function() {
              if (file_globals!=null) {
                if (i18n_cache!=null && (data_cached = i18n_cache.get(''+file_globals))!=null) {
                  data_file_globals = data_cached;
                  step2();
                } else {
                  if (gv.file_exists(file_globals)==false)  { if (i18n_cache!=null)  i18n_cache.del(''+file_globals); data_file_globals = {}; step2(); return; }
                  spud.deserialize(fs.createReadStream(file_globals), 'properties', function (err, data) {
                    if (err) {
                      step2();
                      return;
                    }
                    if (i18n_cache!=null)  i18n_cache.set(''+file_globals, data);
                    data_file_globals = data;
                    step2();
                  });
                }
              } else step2();
